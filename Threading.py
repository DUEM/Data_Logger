...
def threadFunc(conn, addr):
	...
	############################################################################################################################
			...
			if "," in message1:
				...
				if (message[0]!="")and(message[1]!="")and (int(message[0])<9)and(message[1].isalpha() != 1)and(message[0].isalpha() != 1)and(len(message[1])/2) == int(message[0]):				
					q2.put(message)
					print("message sent")
...
def SendCanMessage(can_frame_fmt, can_id,cansock):
	while 1:
		message = q2.get()
		print(message)
		print(struct.pack(can_frame_fmt, can_id, int(message[0]), (bytes.fromhex(message[1])).ljust(8,b"\x00")))
